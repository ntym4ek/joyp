 ...
 *   id = "extn_commerce_pvz_shipping_method",
 *   label = @Translation("CDEK PVZ Shipping Method"),
 *   description = @Translation("CDEK pickup shipping method."),
 *   status = TRUE
 * )
 */
class CDEKPVZShippingMethod extends ShippingMethodBase {
  ...
  public function __construct(array $configuration, $plugin_id, $plugin_definition, PackageTypeManagerInterface $package_type_manager, WorkflowManagerInterface $workflow_manager) {
    ...
    $this->services['default'] = new ShippingService('default', 'До пункта самовывоза');
  ...
  public function calculateRates(ShipmentInterface $shipment) {
    ...
    $shipping_amount = 0;
    $errors = null;
    $description = 'Выберите пункт выдачи';
...
//          echo $exception->getMessage();
        ...
        $tariff->tariff_code = 136;                                                 // Номер тарифа: Посылка склад-склад (https://api-docs.cdek.ru/63345430.html)
        ...
        if ($result->hasErrors()) {
          ...
          if ($errors[0]["code"] == 'err_pvz_with_tariff_mistake') $description = 'Поблизости нет доступных пунктов выдачи.';
